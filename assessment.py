...
def is_hometown(town):
    """Determines if a town is my hometown"""
    if town == 'Washington':
        return True
    return False
...
def greeter(town,firstname,lastname):
    """Greets the user by first and last name depending on hometown"""
    if is_hometown(town):
        return "Hi, " + create_full_name(firstname,lastname) + ", we're from the same place!"
    return "Hi " + create_full_name(firstname,lastname) + ", where are you from?"
...
def calculate_price(base_price,state,tax=.05):
    """Calculates an item's total cost with taxes and fees given a state"""
    price_with_tax = base_price + (tax*base_price)
    if state == 'CA':
        return price_with_tax + (.03*base_price)
    elif state == 'PA':
        return price_with_tax + 2
    elif state == 'MA':
        if base_price < 100:
            return price_with_tax + 1
...
def list_appender(list,*arg):
    """Returns the list with the arguments provided appended"""
    list.append(arg)
    return list
...
def outer(word):
    def multiply_by_three():
        return word*3
    return set(word, multiply_by_three(word))
